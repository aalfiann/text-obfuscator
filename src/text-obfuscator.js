...
const TextObfuscator = {
  ...
  _remake: function (value, size = 1) {
    size = parseInt(size);
    let i = 0; let len = value.length; const data = this._make(value, size); const datalen = data.length; const chunks = new Array(datalen); const last = data[datalen - 1].length;
    for (;i < datalen; i++) {
      if (len >= size) {
